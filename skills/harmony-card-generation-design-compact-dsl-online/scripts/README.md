...
下文命令默认在 `skills/harmony-card-generation-design-compact-dsl-online/` 目录下运行；如果在仓库根目录运行，把 `python scripts/validate_card.py` 替换为 `python skills/harmony-card-generation-design-compact-dsl-online/scripts/validate_card.py`。

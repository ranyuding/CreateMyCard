...
先从目标卡片最近一次工具业务 payload 取得真实 `artifactUrl`，并确认当前运行时 `generateWidgetCardCompactDsl` schema 已声明 `sourceArtifactUrl`。未声明或无法取得真实 URL 时停止编辑，不得改走 create。
...
5. 重新校验全部 `arguments`、`writeResultTo` 和可选 `candidateOutputFields` 后，将完整数组传给 `generateWidgetCardCompactDsl`；删除全部动态数据时传 `[]`。
...
create 模式调用 `generateWidgetCardCompactDsl` 时必须传 `title` 和 `description`，作为最终 CardSpec 的静态展示文案建议。edit 模式仅在用户明确修改对应文案时传入，否则省略并继承来源 CardSpec。
...
- 先确认当前运行时 `generateWidgetCardCompactDsl` schema 已声明 `candidateDataBindings`；未声明时不传。schema 将数组项写成 `Object` 时，每一项按内部 `CandidateDataBinding` 类结构传入，但该内部结构不得用于扩展工具 `arguments` 顶层字段。

...
条件：`generateWidgetCardCompactDsl` 返回 `status: "success"`，存在有效 `artifactUrl`，且本轮没有已知的用户提及数据缺失。
...
- `generateWidgetCardCompactDsl` 返回 `status: "degraded"` 且存在有效 `artifactUrl`；或
...
条件：`generateWidgetCardCompactDsl` 返回 `status: "unsupported"`。
...
- `generateWidgetCardCompactDsl` 返回 `status: "failed"`。
